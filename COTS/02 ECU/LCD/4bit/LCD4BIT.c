...
void LCD_writechar(u8 data) {
	...
	DIO_voidSethalfPortvalV2(DATA_PORT,PORT_HIGH, upp_nibb(0x00 ,data)    ) ;
	DIO_voidSetPinValue(CONTROL_PORT,EN , GPIO_HIGH);
	_delay_ms(10) ;
	DIO_voidSetPinValue(CONTROL_PORT,EN ,GPIO_LOW) ;
	_delay_ms(10) ;
	DIO_voidSethalfPortvalV2(DATA_PORT,PORT_HIGH, low_nibb(0x00 ,data)    ) ;
	DIO_voidSetPinValue(CONTROL_PORT,EN , GPIO_HIGH);
	_delay_ms(10) ;
	DIO_voidSetPinValue(CONTROL_PORT,EN ,GPIO_LOW) ;
	_delay_ms(10) ;
...
void LCD_writecmd(u8 cmd)  {
	...
	//WRITE ON PORTS
		...
		DIO_voidSethalfPortvalV2(DATA_PORT,PORT_HIGH,upp_nibb(0x00,cmd)     ) ;
		DIO_voidSetPinValue(CONTROL_PORT,EN , GPIO_HIGH);
		_delay_ms(10) ;
		...
		DIO_voidSethalfPortvalV2(DATA_PORT,PORT_HIGH,low_nibb(0x00,cmd)     ) ;
		DIO_voidSetPinValue(CONTROL_PORT,EN , GPIO_HIGH);
		_delay_ms(10) ;
		DIO_voidSetPinValue(CONTROL_PORT,EN ,GPIO_LOW) ;
		_delay_ms(10) ;
